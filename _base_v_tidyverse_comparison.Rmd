...
::: {.row data-latex=""}
::: {.lcolumn-50 data-latex="{0.48\textwidth}"}
Some people feel more comfortable using base R functions to import data, which can be done with `read.csv()`.
...
You can also inspect you data with `glimpse()` and others.
...
::: {.row data-latex=""}
::: {.lcolumn-50 data-latex="{0.48\textwidth}"}
Filtering in base R requires the `[` accessor to filter the data frame.

```{r, eval = FALSE}
# Use of the $ accessor to filter the variable.
...
::: {.lcolumn-50 data-latex="{0.48\textwidth}"}
```{r, eval = FALSE}
# Pass limited data frame to the subset function.
...
df$retweet_success <- df$retweet_count/df$followers_count
```
:::
...
::: {.row data-latex=""}
::: {.lcolumn-50 data-latex="{0.48\textwidth}"}
To summarize in base R, you will need to leverage generic functions to calculate these statistics. For example, `mean()`, `median()`, `sum()`, etc.

```{r, eval = FALSE}
# Create a data frame with the summary variable.
data.frame(retweet_average = mean(df$retweet_count))
```
:::
...
::: {.rcolumn-50 data-latex="{0.48\textwidth}"}
To summarize data in **dplyr**, use the `summarize()` function to compute a requested summary (e.g., `mean()`, `median()`, `n()`, etc.) and return a data set with 1 observation.

```{r, eval = FALSE}
tb %>%
  dplyr::summarize(retweet_average = mean(retweet_count))
```
:::
:::
...
::: {.row data-latex=""}
::: {.lcolumn-50 data-latex="{0.48\textwidth}"}
Use the `merge()` function to join. Note the `all.x = TRUE` argument that tells R to keep all observations from the first table.
...
::: {.row data-latex=""}
::: {.lcolumn-50 data-latex="{0.48\textwidth}"}
Use the `merge()` function to join. Note the `all.y = TRUE` argument that tells R to keep all observations from the second table.
...
::: {.rcolumn-50 data-latex="{0.48\textwidth}"}
The `inner_join()` function will merge based on `screen_name`.
